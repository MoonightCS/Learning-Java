...
public class Solution {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final double number = Double.parseDouble(reader.readLine().trim());
        final double finalNumber = filter(number);
        if (Double.compare(finalNumber, 3.0) >= 0) {
            if (Double.compare(finalNumber, 4.0) >= 0)
    ...
    private static double filter(final double number) {
        return number % 5;
    }
}